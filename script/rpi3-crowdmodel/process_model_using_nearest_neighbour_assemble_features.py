...
def main(i):
    ...
    for g in gcc:
        ...
        r1=ck.save_json_to_file({'json_file':'process_model_using_nearest_neighbour_assemble_features/prepare_reactions_model_train_ref_result.'+gx+'.json',
                                'dict':r})
        ...
        r1=ck.save_json_to_file({'json_file':'process_model_using_nearest_neighbour_assemble_features/prepare_reactions_model_train_reduced_result.'+gx+'.json',
                                'dict':r})
        if r1['return']>0: ck.err(r1)
        ...
        r=ck.save_text_file({'text_file':'process_model_using_nearest_neighbour_assemble_features/log.'+gx+'.txt', 'string':s})
        if r['return']>0: return r

        r=ck.save_text_file({'text_file':'process_model_using_nearest_neighbour_assemble_features/influential_features.'+gx+'.txt', 'string':s1})
        if r['return']>0: return r
